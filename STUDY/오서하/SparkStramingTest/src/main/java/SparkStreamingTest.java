...
//        ssc.awaitTermination();
        ...
        JavaInputDStream<ConsumerRecord<String, JSONObject>> stream = KafkaUtils.createDirectStream(
                ssc, LocationStrategies.PreferConsistent(), ConsumerStrategies.<String, JSONObject>Subscribe(topics, kafkaParams));

        stream.mapToPair(
                new PairFunction<ConsumerRecord<String, JSONObject>, String, String>() {
                    public Tuple2<String, String> call(ConsumerRecord<String, JSONObject> record) {
                        JSONObject object = new JSONObject(record.value());
                        String nn = object.get("label").toString();
                        return new Tuple2<String, String>(record.key(), nn);
                    }
                });
